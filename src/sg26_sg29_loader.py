...
data_dir = Path(BASE_DIR) / "sg_loader"
...
patterns = [
    str(input_folder / "*SG26*.csv"),
    str(input_folder / "*SG29*.csv"),
    str(input_folder / "*SG26*.xlsx"),
    str(input_folder / "*SG29*.xlsx"),
]
...
log_action("===== SG26/SG29 Loader script started =====")
...
if not input_files:
    print("No SG26 or SG29 input files found.")
    log_action("No SG26 or SG29 input files found.")
else:
    for input_file in input_files:
        filename = os.path.basename(input_file)
        if "SG26" in filename and (filename.endswith(".csv") or filename.endswith(".xlsx")):
            table_name = "mis_sg26"
        elif "SG29" in filename and (filename.endswith(".csv") or filename.endswith(".xlsx")):
            table_name = "mis_sg29"
        else:
        ...
        ccc = str(df['GI01_DISTRICT_COLLEGE_ID'].iloc[0])
        ttt = str(df['GI03_TERM_ID'].iloc[0])

        # Delete existing records for this campus/term
        delete_sql = f'''
            DELETE FROM "{table_name.upper()}"
            WHERE "GI01_DISTRICT_COLLEGE_ID" = :ccc AND "GI03_TERM_ID" = :ttt
        '''
        ...
        sg_type = "SG26" if table_name == "mis_sg26" else "SG29"
        new_filename = f"{ccc}_{ttt}_{sg_type}{ext}"
...
print("🎉 All done!")
log_action("All done!")
log_action("===== SG26/SG29 Loader script finished =====")